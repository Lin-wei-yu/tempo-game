...
TempoHeart::TempoHeart(ALLEGRO_BITMAP* heart_img, ALLEGRO_BITMAP* beat_marker_img, ALLEGRO_BITMAP* missed_img){
    ...
    int bias = 0;
    for (int i=0; i<4; i++){
        beat_marker_pos.push_back(beat_marker_range*i + bias);
    }
}
...
void TempoHeart::draw(){
    ...
    // set heart parameters.
    int heart_w = al_get_bitmap_width(img)/2;
    int heart_h = al_get_bitmap_width(img);
    int heart_scaled_ratio = 3;
    int heart_offset_x = 12;
    
    // draw beat marker 
    int marker_w = al_get_bitmap_width(beat_marker_img);
    int marker_h = al_get_bitmap_height(beat_marker_img);
    int marker_offset_x = 100;
    int marker_offset_y = 30;
    int range_per_beat = beat_marker_range / BEAT_PER_TEMPO;
    for(auto pos : beat_marker_pos){
        int left_bar_pos = marker_offset_x+pos+(range_per_beat*beat_cnt);
        int right_bar_pos = WINDOW_WIDTH-left_bar_pos;

        int half_heart_w = (heart_w*heart_scaled_ratio)/2;

        if ( left_bar_pos >= (pos_x)){
             al_draw_scaled_bitmap(beat_marker_img, 0, 0, marker_w, marker_h, pos_x, pos_y+marker_offset_y, marker_w*3, marker_h*3, 0);
        }else{
            al_draw_scaled_bitmap(beat_marker_img, 0, 0, marker_w, marker_h, left_bar_pos, pos_y+marker_offset_y, marker_w*3, marker_h*3, 0);
        }
         if (right_bar_pos <= pos_x){
             al_draw_scaled_bitmap(beat_marker_img, 0, 0, marker_w, marker_h, pos_x, pos_y+marker_offset_y, marker_w*3, marker_h*3, 0);    
        }else {
            al_draw_scaled_bitmap(beat_marker_img, 0, 0, marker_w, marker_h, right_bar_pos, pos_y+marker_offset_y, marker_w*3, marker_h*3, 0);    
        }
    }
    // draw bumping heart
    al_draw_scaled_bitmap(img, heart_w*cur_animation, 0, heart_w, heart_h, pos_x-(heart_w*heart_scaled_ratio/2)+heart_offset_x, pos_y, heart_w*heart_scaled_ratio, heart_h*heart_scaled_ratio, 0);
